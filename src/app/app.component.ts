...
export class AppComponent implements OnInit, OnDestroy{
  ...
  constructor(private firestoreService: FirestoreService, 
    ...
    this.subscription = this.projectService.getProject().subscribe(project => {
      if (project) {
    ...
    this.openSubscription = this.projectService.openProject().subscribe(async project => {
      if (project) {
        await wait(20);
    ...
    this.animationsSubscription = this.dataService.getAnimations().subscribe(animations => {
      if (animations) {
 ...
 ngOnInit() {
  ...
  projectChanged(project: any) {
    const {width, height, top, left} = this.childrenFolder.toArray()[project.openIndex]
